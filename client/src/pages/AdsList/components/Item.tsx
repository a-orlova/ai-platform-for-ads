...
export default function Item({ id, title, price, category, needsRevision, imageUrl, viewMode }: ItemProps) {
  const navigate = useNavigate()

  const categoryPlaceholder =
    category === 'real_estate' ? BuildingPlaceholder : category === 'auto' ? AutoPlaceholder : ElectronicsPlaceholder
  ...
  return (
    <div className={`ad-item ${viewMode === 'list' ? 'ad-item-list' : ''}`} onClick={() => navigate(`/ads/${id}`)}>
      <div className="ad-item-photo">
        ...
        />
        <p className="category-txt">{categoryLabel}</p>
      </div>
